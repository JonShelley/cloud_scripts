...
XID_EC = {"48": {"description": "Double Bit ECC Error", "severity": "Critical"}, 
...
if args.dmesg_cmd != None:
    logger.debug(f"Using dmesg command: {args.dmesg_cmd}")
    dmesg_cmd = args.dmesg_cmd

def check_gpu_xid():
    results = {}
    status = "Pass"
    dmesg_output = subprocess.check_output([dmesg_cmd]).decode("utf-8")
    if "NVRM: Xid" in dmesg_output:
        for XID in XID_EC.keys():
            ...
            for x in tmp_dict.keys():
                logger.info(f"{GPU_XID_TEST}: {XID} : count: {tmp_dict[x]}, {XID_EC[XID]['description']} - PCI: {x}")
            ...
            if tmp_dict != {}:
                if XID_EC[XID]['severity'] == "Critical":
                    status = "Fail"
                results[XID] = {"results": tmp_dict, "description": XID_EC[XID]['description']}
    else:
        logger.info("No GPU Xid error found in dmesg")
    return {"status": status, "results": results}

results = check_gpu_xid()
print(results)